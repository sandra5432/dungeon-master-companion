...
| AL-B1-015 | Epochen auf Zeitleiste anzeigen | Ereignisse, die einer Epoche zugeordnet sind, werden in der Zeitleiste durch ein farbiges Band mit vertikalem Namensstreifen links neben den Karten visuell gruppiert. Jede Epoche hat eine individuelle Farbe. Offene Epochen (ohne definierten Endpunkt) sind durch ein ⋯-Symbol am oberen Rand des Streifens gekennzeichnet. | Leseberechtigung (konfigurierbar) |
| AL-B1-016 | Epoche anlegen | Über den „+"-Button im Epochen-Bereich der rechten Sidebar kann eine neue Epoche angelegt werden. Pflichtfelder: Name, erstes Ereignis (Startpunkt), Farbe (aus einer festen 7-Farben-Palette). Optionales Feld: letztes Ereignis (Endpunkt); bleibt es leer, gilt die Epoche als offen. Ereignisse, die bereits einer anderen Epoche angehören, werden in den Dropdowns nicht angezeigt. Eine Vorschau zeigt die Anzahl der abgedeckten Ereignisse. | Schreibberechtigung (konfigurierbar) |
| AL-B1-017 | Epoche bearbeiten | Eine bestehende Epoche kann über den ✎-Button am Epochenband in der Zeitleiste oder in der rechten Sidebar geöffnet und bearbeitet werden. Alle Felder (Name, Farbe, Start, Ende) sind änderbar. Ereignisse anderer Epochen bleiben aus den Dropdowns ausgeblendet; die eigenen Ereignisse der Epoche bleiben auswählbar. | Schreibberechtigung (konfigurierbar) |
| AL-B1-018 | Epoche löschen | Eine Epoche kann über den ✕-Button am Epochenband in der Zeitleiste oder in der rechten Sidebar gelöscht werden. Nach einer Bestätigung wird nur die Epochen-Markierung entfernt; die enthaltenen Ereignisse bleiben unverändert erhalten. | Schreibberechtigung (konfigurierbar) |
| AL-B1-019 | Epoche ein-/ausklappen | Jedes Epochenband kann über den ▼/▶-Button im linken Streifen ein- oder ausgeklappt werden. Im eingeklappten Zustand werden die Ereignisse ausgeblendet; der Streifen zeigt Epochenname und Anzahl der enthaltenen Ereignisse. Der Zustand wird im lokalen Browser-Speicher persistiert und beim nächsten Besuch wiederhergestellt. | Leseberechtigung (konfigurierbar) |
