...
| AL-D-006 | Standard-Tags | Beim Erstellen einer Idee stehen vorkonfigurierte Welt-Tags als Schnellauswahl zur Verfügung (z. B. „pardur", „eldorheim"). Klick auf einen Chip fügt den Tag in das Tags-Feld ein.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Eingeloggte Nutzer |
| AL-D-010 | Deep Links | Jede Idee hat eine eigene URL: `/ideas` für das Board, `/ideas/{id}` für eine geöffnete Idee. Beim direkten Aufruf einer solchen URL lädt das Board und öffnet automatisch das zugehörige Detail-Panel. Klick auf eine Karte aktualisiert die URL; Schließen des Panels setzt sie auf `/ideas` zurück. Die Browser-Zurück-Taste schließt das Panel und navigiert zu `/ideas`. Gäste werden beim Aufrufen einer Ideenkammer-URL auf die Startseite umgeleitet.                                                                                                                                                                                                                                                                                                               | Eingeloggte Nutzer |
...
| Feature ID | Feature Name | Description | Access |
|------------|--------------|-------------|--------|
| AL-D5-001 | Bild hochladen | Im Detail-Panel zeigt ein „+ Bild anhängen"-Button für Ersteller und Admins ein verstecktes Dateiauswahl-Input. Es werden ausschließlich WebP-Dateien akzeptiert (max. 5 MB). Das Bild wird per Multipart-POST an den Server übermittelt und erscheint sofort als Vorschaubild in der Galerie im Detail-Panel. Die Bildanzahl wird auf der Ideenkarte mit einem `🖼 N`-Badge angezeigt. | Ersteller oder Admin |
| AL-D5-002 | Bild löschen | Jedes Vorschaubild in der Galerie des Detail-Panels hat ein ✕-Schaltfläche. Ein Klick darauf löscht das Bild dauerhaft (Bestätigung nicht erforderlich) und entfernt es sofort aus der Galerie. Die Bildanzahl auf der Karte wird aktualisiert. | Ersteller oder Admin |
| AL-D5-003 | Bilder beim Erstellen hochladen | Im Idee-erstellen-Modal gibt es einen „📎 Bilder hinzufügen"-Button (verstecktes `<input type="file" multiple accept="image/webp">`). Ausschließlich WebP-Dateien (max. 5 MB je Datei) werden akzeptiert. Ausgewählte Bilder erscheinen als Vorschau-Thumbnails unterhalb; jede Vorschau hat ein ✕ zum Entfernen. Nach dem Speichern der Idee werden die vorgemerkten Bilder sequentiell hochgeladen; die fertig angelegte Idee zeigt die Bilder sofort im Detail-Panel an. Im Bearbeiten-Modal ist der Bildabschnitt nicht sichtbar (dort nutzt man den Hochladen-Button im Detail-Panel). | Eingeloggte Nutzer |
| AL-D5-004 | Mini-Bildvorschau auf der Ideenkarte | Hat eine Idee mindestens ein Bild, wird das erste Bild als kleines Thumbnail auf der Ideenkarte selbst angezeigt. Die Karte wechselt dabei in ein zweispaltiges Layout: Thumbnail links, Inhaltsbereich rechts. Das Thumbnail ist nicht eigenständig klickbar; der Klick auf die Karte öffnet wie gewohnt das Detail-Panel. | Eingeloggte Nutzer |
